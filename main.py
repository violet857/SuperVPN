...
def run_vpn_command(select_server):
    ...
    args = ["", "client", "vpn.key", select_server['ip'], select_server['port']] 
    # print(f"Starting VPN process with args: {args}")
    try:
       vpn_process = multiprocessing.Process(target=run_vpn, args=(args,))
       vpn_process.start()
...
def on_closing():
    if vpn_process:
        vpn_process.terminate()
        vpn_process.join(timeout=5)
        if vpn_process.is_alive():            
            vpn_process.kill()
...
if __name__ == '__main__':
    ...
    # Populate the treeview with the new server names and add a checkmark to the status
    item_ids = {}
    for server in servers:
        # Assume that all servers are up and running hence we add a checkmark
        item_id = tree.insert('', 'end', values=(server['name'], server['usage'], server['latency']))
        item_ids[server['name']] = item_id

    # Run status update (you may want to call this periodically or after certain actions)
