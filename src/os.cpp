...
// 简化版，读取最多 max_count 字节的数据到指定的缓冲区中
...
// 创建一个TUN virtual network interface，返回文件描述符
#ifdef __linux__
int tun_create(char if_name[IFNAMSIZ], const char *wanted_name)
{
    ...
    fd = open("/dev/net/tun", O_RDWR); // O_RDWR: 读写方式打开
    if (fd == -1) {
...
// 设置TUN virtual network interface的MTU
...
// executing a shell command and reading its output
...
// 获取默认的外部服务的名称系统网络接口，用于正确配置流量转发或分割隧道
...
// 拥塞控制算法
    ...
    (void) setsockopt(fd, IPPROTO_TCP, TCP_NOTSENT_LOWAT, &notsent_lowat, sizeof(notsent_lowat));
#endif
...
// #endif
    ...
    (void) setsockopt(fd, IPPROTO_TCP, TCP_USER_TIMEOUT, reinterpret_cast<char*>(&timeout), sizeof(unsigned int));
#endif
#ifdef SO_MARK
    unsigned int markValue = 42069U;
    if (setsockopt(fd, SOL_SOCKET, SO_MARK, &markValue, sizeof(markValue)) != 0) {
        // 处理错误
    }
#endif
    return 0;
