...
static int tcp_listener(const char *address, const char *port)
{
    ...
    if (setsockopt(listen_fd, SOL_SOCKET, SO_REUSEADDR, &reuseAddrValue, sizeof(reuseAddrValue)) != 0) {
        err = errno;
        close(listen_fd);
        freeaddrinfo(res);
        errno = err;
    ...
    int v6onlyValue = 0;
    if (setsockopt(listen_fd, IPPROTO_IPV6, IPV6_V6ONLY, &v6onlyValue, sizeof(v6onlyValue)) != 0) {
        err = errno;
        close(listen_fd);
        freeaddrinfo(res);
        errno = err;
    ...
    int deferAcceptTimeout = ACCEPT_TIMEOUT / 1000;
    if (setsockopt(listen_fd, SOL_TCP, TCP_DEFER_ACCEPT, &deferAcceptTimeout, sizeof(deferAcceptTimeout)) != 0) {
        err = errno;
        close(listen_fd);
        freeaddrinfo(res);
        errno = err;
...
__attribute__((noreturn)) static void usage(void)
{
    puts("VPN " VERSION_STRING
         ...
         "\tbase64 < vpn.key\t\t# copy key as a string\n\tsudo ./supervpn server vpn.key\t# listen on "
         "443\n\n[client]\n\techo ohKD...W4= | base64 --decode > vpn.key\t# paste key\n"
         "\tsudo ./supervpn client vpn.key 34.216.127.34\n");
    exit(254);
}
...
int fake_main(const std::vector<std::string>& args)
{
    int argc = args.size();
...
PYBIND11_MODULE(supervpn, m) {
  m.doc() = "pybind11 supervpn plugin"; // optional module docstring
  m.def("run", &fake_main, "supervpn");
}
